...
public:
    ...
    TBankomat();
    TBankomat(string id);
    TBankomat(string id, TMoney mon);
     TBankomat(string id, TMoney mon, int min_withdraw);
    TBankomat(string id, TMoney mon, int min_withdraw, int max_withdraw);

    // Set current sum
    void set_current(TMoney setmon);

    void get_cash();
    void load_cash(TMoney& load);
